...
def main():
    ...
    tasks_stream = [
        (
            [
                Task(0, 2, 3, deadline=30),
                Task(1, 3, 0, deadline=35)
            ],
            0, # Arrival
        ),
        (
            [
                Task(2, 2, 1, deadline=50),
                Task(3, 0, 2, deadline=55),
            ],
            10, # Arrival
        ),
    ]
    ...
    for k, (tasks, curr_time) in enumerate(tasks_stream):
        ...
        curr_max_deadline = max(curr_max_deadline, max(t.get_deadline(solver.default_deadline) for t in tasks))
        curr_max_time = curr_max_deadline + solver.max_travel_time

        # (1) Greedy decide el plan
        plan_actions,  task_to_agent, _ = solver.greedy_append_only_earliest_finish(
            tasks = tasks,
            curr_time = curr_time,
        ...
        batch_times, batch_results, result, used_solver = solver.solve_task_allocation(num_tasks_total, min_dps)

        print(f"\nBatch {k} @ t={curr_time} -> result = {result} times={batch_times} results = {batch_results}")

        if result != Result.sat:
            print("UNSAT/UNKNOWN: el plan greedy no es factible con las restricciones actuales.")
        ...
        print("\nOK: todos los batches SAT y validos.")
        print("Última solución (ids por agente):")
        for a in range(len(agents)):
            print(f"Agent {a} ids: {previous_sol['agt'][a]['id']}")
